    ...
    <div className="w-full max-w-2xl mx-auto bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <div
          ...
          onDrop={handleDrop}
          className={`relative border-2 border-dashed rounded-xl p-8 flex flex-col items-center justify-center min-h-[260px] transition duration-200 ${
            dragActive ? 'border-emerald-400 bg-emerald-500/5' : 'border-slate-700 bg-slate-950/50 hover:border-slate-600'
          }`}
        >
          <input
            ref={fileInputRef}
          ...
          {!preview ? (
            <div className="flex flex-col items-center text-center cursor-pointer w-full py-8" onClick={() => fileInputRef.current?.click()}>
              ...
              <p className="text-slate-200 font-medium mb-1">
                Drag and drop image here, or <span className="text-emerald-400 hover:text-emerald-300">browse</span>
              </p>
            ...
            <div className="relative w-full max-h-[300px] flex justify-center items-center overflow-hidden rounded-lg">
              ...
              {!isLoading && (
                <button
                  type="button"
                  onClick={handleReset}
                  className="absolute top-2 right-2 bg-slate-950/80 hover:bg-slate-900 text-white rounded-full p-1.5 hover:scale-105 transition"
                  title="Remove image"
                >
          ...
          <button
            ...
            className={`w-full py-3 px-6 rounded-xl font-semibold text-slate-950 text-center transition duration-200 flex items-center justify-center gap-2 ${
              isLoading || !file
                ...
                : 'bg-gradient-to-r from-emerald-400 to-teal-400 hover:shadow-lg hover:shadow-emerald-500/20 active:scale-[0.99]'
            }`}
          >
