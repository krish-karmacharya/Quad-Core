...
const analyzeImage = async (req, res, next) => {
  try {
    ...
    // 1. Run Roboflow smoke detection (or use frontend-computed results)
    let smokeResult;
    if (req.body.smokeDetection) {
      try {
        smokeResult = typeof req.body.smokeDetection === 'string'
          ? JSON.parse(req.body.smokeDetection)
          : req.body.smokeDetection;
        console.log('Using frontend-computed smoke detection results:', smokeResult);
      } catch (err) {
        console.error('Failed to parse frontend smoke detection results:', err);
      }
    }
