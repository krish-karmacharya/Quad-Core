...
const ResultCard = ({ result }) => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      case 'PLATE_DETECTED':
        return <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">Plate Detected</span>;
      case 'PLATE_NOT_FOUND':
        return <span className="bg-orange-500/10 text-orange-400 border border-orange-500/20 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">No Plate Found</span>;
      case 'DETECTION_FAILED':
        return <span className="bg-rose-500/10 text-rose-400 border border-rose-500/20 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">Detection Failed</span>;
      default:
        return <span className="bg-slate-500/10 text-slate-400 border border-slate-500/20 px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wider">Unknown</span>;
    }
  };
